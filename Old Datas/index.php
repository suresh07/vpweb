	...
	<div class="body">
		<div class="column1">
			<?php 
					...
					$result=mysql_query("select * from article where volume=$volume and issuenum=$issue and feature='ಸಂಪಾದಕೀಯ' limit 1");
					$row=mysql_fetch_assoc($result);
	
					echo "<div class=\"widget\">"; 
					echo "<div class=\"tbar\">".$month[$row['issuenum']-1]." ಸಂಚಿಕೆ</div>";
					...
					echo "<span class=\"text\"><a href=\"/Volumes/".$row['volume']."/".$row['issuenum']."/index.djvu?djvuopts&page=".$row['page']."&zoom=page\" target=\"_blank\">ಸಂಪಾದಕೀಯ: ".$row['title']."</a></span>";
					echo "</div>";
			?>
		</div>
...
<?php
	function print_widget($feature)
	{
		...
		$query="select * from article where feature='$feature' and volume=$volume and issuenum=$issue";
		...
		if($num_rows>0)
		{
			for($i=0;$i<$num_rows;$i++)
			{
				...
				echo "<div>";
				echo "<img id=\"art_widget_img\" src=\"Volumes/".$row['volume']."/".$row['issuenum']."/images/".$row['page'].".png\" alt=\"cover\"/>";
				echo "</div>";
				echo "<div style=\"width: 50%;\" class=\"text\">";
				echo "<span class=\"titlespan\"><a href=\"Volumes/".$row['volume']."/".$row['issuenum']."/index.djvu?djvuopts&page=".$row['page']."&zoom=page\" target=\"_blank\">".$row['title']."</a></span><br />";
				echo "<span class=\"authorspan\"><a href=\"html/auth.php?authid=".$authid."\" target=\"_blank\">".$author.", ".$row1['salutation']."</a></span><br />";
				echo $row['text']."</div>";
				echo "<div class=\"further\"><span class=\"furtherspan\"><a href=\"Volumes/".$row['volume']."/".$row['issuenum']."/index.djvu?djvuopts&page=".$row['page']."&zoom=page\" target=\"_blank\">ಮುಂದೆ ಓದಿ..</a></span></div>";
				if($i!=$num_rows-1)
				{
